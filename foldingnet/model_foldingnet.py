...
class DGCNN_Cls_Encoder(nn.Module):
    ...
    def forward(self, x):        
        ...
        x2 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points)    hidden1
        ...
        x4 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 256, num_points, k) -> (batch_size, 256, num_points)    hidden2
        ...
        feat = x.unsqueeze(1)                   # (batch_size, feat_dims) -> (batch_size, 1, feat_dims)                bottleneck
...
class FoldNet_Encoder(nn.Module):
    def __init__(self, args):
        ...
        if args.k == None:
            ...
            self.k = args.k
        self.n = 5120   # input point cloud size
...
class FoldNet_Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.meshgrid = [[-0.3, 0.3, 71], [-0.3, 0.3, 71]]
